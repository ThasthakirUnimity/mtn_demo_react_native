...
const Picker = ({ variant = 'default', ...props }) => {
  ...
  const { onChange, placeholder /* placeholderTextColor, style: _style, ...p */ } = props

  useEffect(() => {
    const s = props.items.find(r => r.value == props.value)
    setSelected(s || null)
  }, [props.items])
  useEffect(() => {
    ...
    return props.renderLabel
      ...
      : (
        <View style={props.labelContainerStyle}>
          <Text style={props.labelStyle || styles.labelStyle}>{selected.label}</Text>
        </View>
        )
  }
  ...
  const renderArrow = () => {
    ...
    return (
      <View style={props.iconContainerStyle}>
        <Icon name={props.iconName || 'chevron-small-down'} type={props.iconType || 'Entypo'} style={props.iconStyle || styles.iconStyle} />
      </View>
    )
  }
  ...
  const renderView = () => {
    return (
      <Button style={props.buttonStyle || styles.buttonStyle} onPress={openPicker}>
  ...
  const renderItem = (item, isFirst, isLast) => {
    ...
    return (
      <Button
        ...
        onPress={selectItem}
      >
        <View style={props.optionInnerStyle}>
          {content}
        </View>
      </Button>
    )
  ...
  const renderItems = () => {
    if (props.items && props.items.length) {
      ...
      return props.items.map((item, index) => renderItem(item, index === 0, index === (length - 1)))
    }
    ...
    return (
      <View style={styles.header}>
        ...
        <View style={styles.headerContent}>
          <Text style={styles.headerText}>{props.optionListTitle || placeholder || 'Select'}</Text>
        </View>
      </View>
    )
  }
