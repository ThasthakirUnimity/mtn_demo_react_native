...
import { logScreenChange } from '@src/utility/analytics'
import { exitApp } from '@src/utility/core'
// import {onScreenChange as onScreenChangeSupportChat} from '@src/utility/supportChat'
...
export const navigateCurrent = async (routeName, params = {}, excludes = []) => {
...
export const onLastScreenLeave = (e) => {
  ...
  if (state && state?.routes && state.routes.length === 1) {
    if (state.routes[0].name == 'DrawerNav' || state.routes[0].name == 'UserLogin') {
      ...
      return false
    } else if (store.getState().session.isLoggedIn) {
      navigate('PublicHome')
      return false
    } else {
      exitApp()
...
export const onStateChange = async () => {
  ...
  if (previousRouteName !== currentRouteName) {
    logScreenChange(currentRouteName)
    // onScreenChangeSupportChat(currentRouteName)
  }
