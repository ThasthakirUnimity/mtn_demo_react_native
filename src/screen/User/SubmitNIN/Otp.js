...
class Otp extends React.Component {
  ...
  async onSubmit () {
    ...
    try {
      ...
      const result = (await http.post(URLS.USER_NIN_VERIFY, values)).data

      logSuccessEvent('SubmitNINOtp')
    ...
    return (
      <View style={styles.otpContent}>
        <View style={styles.otpHeader}>
          <Text style={styles.otpTitle}>{__('OTP')}</Text>
          <Text style={styles.otpDesc}>{__('OTP has been sent to your mobile number')}</Text>
        </View>
        <View style={styles.otpForm}>
          {/* }<Text style={styles.otpTime}>{__('Time left:')} 59 sec</Text>{ */}
          <View style={styles.otpFormContent}>
            {this.renderCodeInput()}
          </View>
        </View>
        {/* }
        <Button onPress={undefined}>
          <Text style={styles.otpResendtext}>{__('Don’t receive the OTP? Resend OTP')}</Text>
        </Button>
        { */}
      </View>
    )
