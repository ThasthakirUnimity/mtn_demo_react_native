...
const imagePicker = (method, onSuccess, onError, options = {}) => {
  ...
  ImagePicker[method](options).then((r) => {
...
export const openImagePicker = (options = {}) => {
  ...
  Alert.alert(
    ...
    [
      ...
      { text: 'Gallery', onPress: () => imagePicker('openPicker', onSuccess, onError, ooptions) },
      { text: 'Camera', onPress: () => imagePicker('openCamera', onSuccess, onError, ooptions) }
    ],
    { cancelable: false }
  )
}

export const openVideoPicker = (options = {}) => {
  const defaultOptions = {
    width: 500,
    height: 500,
    cropping: false,
